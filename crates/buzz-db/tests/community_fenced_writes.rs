...
struct ParserException {
    file: &'static str,
    owner: &'static str,
    sql_fingerprint: &'static str,
}

// PostgreSQL syntax intentionally unsupported by sqlparser 0.62. Each entry is
// an exact file/owner/statement fingerprint; source or relocation edits fail
// closed until the statement either parses or this inventory is deliberately
// updated.
const PARSER_EXCEPTIONS: &[ParserException] = &[
    ParserException {
        file: "crates/buzz-db/src/deletion.rs",
        owner: "claim",
        sql_fingerprint: "c1e18a023ece8e46245ad770c57f142d02295c07c22b874658f0e800bd1bcce5",
    },
    ParserException {
        file: "crates/buzz-db/src/push.rs",
        owner: "claim_due_match_batch_with_loader",
        sql_fingerprint: "e2faa0183bccd8c8e702a8beb756bb795e356484798b3f57f2c763c8a2c4df78",
    },
    ParserException {
        file: "crates/buzz-db/src/push.rs",
        owner: "claim_due_wakes",
        sql_fingerprint: "4078b15d1bf7c6359418d52c8c60b9693cec3dd2e715028ae2188c927d69baf6",
    },
    ParserException {
        file: "crates/buzz-db/src/lib.rs",
        owner: "ensure_configured_community",
        sql_fingerprint: "23ac6c2ee1e455dd174b99110142388dd84691fa3bd20be4fd651cac1c0346aa",
    },
    ParserException {
        file: "crates/buzz-db/src/lib.rs",
        owner: "create_community_with_owner",
        sql_fingerprint: "57f23268f62baf65eb14b753d1f4be3a34cff89c45694f3738183b703d94be4e",
    },
];
...
fn sql_literal_from_definition(definition: &serde_json::Value) -> Option<String> {
    definition["metaVariables"]["single"]
        .get("VALUE")?
        .get("text")?
        .as_str()
        .and_then(rust_string_value)
}

fn conditional_sql_literals(definition: &serde_json::Value) -> Option<[String; 2]> {
    let single = &definition["metaVariables"]["single"];
    let then_sql = single.get("A")?.get("text")?.as_str()?;
    let else_sql = single.get("B")?.get("text")?.as_str()?;
    Some([rust_string_value(then_sql)?, rust_string_value(else_sql)?])
}

fn literal_sql_violations(sql: &str, fenced: &BTreeSet<String>) -> Result<Vec<String>, String> {
    let statements = parse_sql(sql)?;
    let (_, violations) = mutation_violations(&statements, fenced);
    Ok(violations)
}
...
fn exact_fingerprint(source: &str) -> String {
...
fn inspect_matches_with_owners(
    ...
    inspect_matches_with_inventories(matches, roots, owners, &const_defs, &conditional_defs)
}

fn inspect_matches_with_inventories(
    matches: &[serde_json::Value],
    roots: &[PathBuf],
    owners: &[FunctionOwner],
    const_defs: &[serde_json::Value],
    conditional_defs: &[serde_json::Value],
) -> Vec<String> {
    ...
    for matched in matches {
        ...
        match parse_sql(&sql) {
            ...
            {
                let relative = path.strip_prefix(&root).unwrap_or(&path).to_string_lossy();
                let sql_fingerprint = exact_fingerprint(&sql);
                let owner = owners.iter().find(|owner| {
                    owner.path == path && owner.start <= offset && offset < owner.end
                });
                let exception_count = PARSER_EXCEPTIONS
                    .iter()
                    .filter(|entry| {
                        relative == entry.file
                            && sql_fingerprint == entry.sql_fingerprint
                            && owner.is_some_and(|owner| owner.name == entry.owner)
                    })
                    .count();
                if exception_count != 1 {
                    violations.push(format!(
                        "{relative}:{line}: production write SQL did not parse and must match exactly one parser exception; got {exception_count}; fingerprint={sql_fingerprint}: {error}"
                    ));
                }
            }
            Err(_) => {}
    ...
    for (relative, symbol, expression) in NAMED_SQL {
        ...
        let executions = matches
            ...
            .count();
        if definitions.len() != 1 || executions == 0 {
            violations.push(format!("{relative}: named SQL inventory mismatch for {symbol}: definitions={} executions={executions}", definitions.len()));
            continue;
        }
        let Some(sql) = sql_literal_from_definition(definitions[0]) else {
            violations.push(format!(
                "{relative}: named SQL inventory for {symbol} must resolve to one string literal"
            ));
            continue;
        };
        match literal_sql_violations(&sql, &fenced) {
            Ok(messages) => violations.extend(
                messages
                    .into_iter()
                    .map(|message| format!("{relative}: named SQL {symbol}: {message}")),
            ),
            Err(error) => violations.push(format!(
                "{relative}: named SQL {symbol} did not parse: {error}"
            )),
        }
    }
    for (relative, expected_owner, expression) in CONDITIONAL_SQL {
        ...
        if definitions.len() != 1 {
            violations.push(format!("{relative}: conditional SQL inventory mismatch: owner={expected_owner} expression={expression} definitions={}", definitions.len()));
            continue;
        }
        let Some(branches) = conditional_sql_literals(definitions[0]) else {
            violations.push(format!(
                "{relative}: conditional SQL inventory for {expected_owner}::{expression} must resolve both branches to string literals"
            ));
            continue;
        };
        for (branch, sql) in branches.iter().enumerate() {
            match literal_sql_violations(sql, &fenced) {
                Ok(messages) => violations.extend(messages.into_iter().map(|message| {
                    format!(
                        "{relative}: conditional SQL {expected_owner}::{expression} branch {branch}: {message}"
                    )
                })),
                Err(error) => violations.push(format!(
                    "{relative}: conditional SQL {expected_owner}::{expression} branch {branch} did not parse: {error}"
                )),
            }
        }
    }
    if roots == production_roots() {
        for entry in PARSER_EXCEPTIONS {
            let path = root.join(entry.file);
            let count = matches
                .iter()
                .filter(|matched| Path::new(matched["file"].as_str().unwrap_or_default()) == path)
                .filter(|matched| {
                    matched["range"]["byteOffset"]["start"]
                        .as_u64()
                        .is_some_and(|offset| {
                ...
                .filter(|sql| {
                    parse_sql(sql).is_err() && exact_fingerprint(sql) == entry.sql_fingerprint
                })
                .count();
            if count != 1 {
                violations.push(format!(
                    "{}: parser exception must match exactly once: owner={:?} fingerprint={:?}; got {count}",
                    entry.file, entry.owner, entry.sql_fingerprint
                ));
            }
        }
    }
...
#[test]
fn scanner_rejects_named_and_conditional_sql_producer_changes() {
    let roots = production_roots();
    let matches = ast_matches(&roots);
    let owners = function_owners(&roots);
    let const_defs = const_sql_definitions(&roots);
    ...
    let mut unsafe_const_defs = const_defs.clone();
    let named_definition = unsafe_const_defs
        .iter_mut()
        .find(|definition| {
            definition["file"].as_str()
                == Some(
                    repo_root()
                        .join("crates/buzz-db/src/reaction.rs")
                        .to_string_lossy()
                        .as_ref(),
                )
                && definition["metaVariables"]["single"]["NAME"]["text"].as_str()
                    == Some("ADD_REACTION_SQL")
        })
        .expect("named SQL definition");
    named_definition["metaVariables"]["single"]["VALUE"]["text"] = serde_json::Value::String(
        r##"r#"DELETE FROM relay_invites WHERE expires_at < now()"#"##.to_string(),
    );
    let violations = inspect_matches_with_inventories(
        &matches,
        &roots,
        &owners,
        &unsafe_const_defs,
    ...
    assert!(
        violations.iter().any(|violation| {
            violation.contains("ADD_REACTION_SQL") && violation.contains("fleet DELETE")
        }),
        "changed named SQL producer escaped: {violations:?}"
    );

    let mut unsafe_conditional_defs = conditional_defs.clone();
    let conditional_definition = unsafe_conditional_defs
        .iter_mut()
        .find(|definition| {
            definition["file"].as_str()
                == Some(
                    repo_root()
                        .join("crates/buzz-db/src/lib.rs")
                        .to_string_lossy()
                        .as_ref(),
                )
                && definition["metaVariables"]["single"]["NAME"]["text"].as_str()
        ...
        .expect("conditional SQL definition");
    conditional_definition["metaVariables"]["single"]["A"]["text"] = serde_json::Value::String(
        r#""DELETE FROM relay_invites WHERE expires_at < now()""#.to_string(),
    );
    let violations = inspect_matches_with_inventories(
        &matches,
        &roots,
        &owners,
        &const_defs,
        &unsafe_conditional_defs,
    );
    assert!(
        violations.iter().any(|violation| {
            violation.contains("replace_parameterized_event::statement branch 0")
                && violation.contains("fleet DELETE")
        }),
        "changed conditional SQL producer escaped: {violations:?}"
    );
}

#[test]
fn scanner_rejects_parser_exception_changes() {
    let roots = production_roots();
    let mut matches = ast_matches(&roots);
    let owners = function_owners(&roots);
    let entry = PARSER_EXCEPTIONS
        .first()
        .expect("at least one reviewed parser exception");
        ...
        .position(|matched| {
            Path::new(matched["file"].as_str().unwrap_or_default()) == path
                && matched["metaVariables"]["single"]
                    ...
                    .and_then(|sql| sql["text"].as_str())
                    .and_then(rust_string_value)
                    .is_some_and(|sql| exact_fingerprint(&sql) == entry.sql_fingerprint)
        })
        .expect("reviewed parser exception execution");
    let original_token = matches[match_index]["metaVariables"]["single"]["SQL"]["text"]
        .as_str()
        .expect("parser exception SQL token");
    let original_sql = rust_string_value(original_token).expect("literal parser exception SQL");
    let changed_sql = original_sql.replacen(
        "WHERE request.id = candidate.id",
        "WHERE request.id = candidate.id OR request.id IS NOT NULL",
        1,
    );
    assert_ne!(original_sql, changed_sql, "calibration must change the SQL");
    matches[match_index]["metaVariables"]["single"]["SQL"]["text"] =
        serde_json::Value::String(format!("{changed_sql:?}"));

    let violations = inspect_matches_with_owners(&matches, &roots, &owners);
    assert!(
        violations.iter().any(|violation| {
            violation.contains(entry.file)
                && violation.contains("must match exactly one parser exception")
        }),
        "changed parser exception escaped: {violations:?}"
    );
}
